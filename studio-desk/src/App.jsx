...
// ---------- design tokens (FOMA brand) ----------
const T = {
  paper: "#F2F1EF",      // light warm grey canvas
  ink: "#1C1C1C",        // brand black (from wordmark)
  muted: "#5C5C5C",
  faint: "#6E6E6E",
  line: "#E4E3DF",
  card: "#FFFFFF",
  accent: "#FF2EE9",     // brand magenta (from icon) — fills, logo, emphasis
  accentInk: "#B01BA0",  // deepened magenta — legible as text/links on light bg
  accentSoft: "#FFE7FB", // pink tint for active/selected surfaces
  danger: "#C4372B",
  dangerSoft: "#FBEEEC",
  ok: "#1E7F4F",
  okSoft: "#EAF5EF",
...
const MONO = "'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";

const STATUSES = ["Prospect", "Preparing", "Submitted", "Interview", "Accepted", "Declined"];
const STATUS_STYLE = {
  Prospect:  { fg: T.muted,     bg: "#EFEEEA" },
  Preparing: { fg: T.accentInk, bg: T.accentSoft },
  Submitted: { fg: T.amber,     bg: T.amberSoft },
  Interview: { fg: "#6D28A8",    bg: "#F1E8FA" },
  Accepted:  { fg: T.ok,        bg: T.okSoft },
  Declined:  { fg: T.danger,    bg: T.dangerSoft },
};

// FOMA icon mark (exact geometry from the brand file)
function FomaMark({ size = 22, color = T.accent }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
      style={{ display: "block", flexShrink: 0 }}>
      {[
        "matrix(1,0,0,-1,36.521,23.542702)","matrix(1,0,0,-1,36.521,36.7818)","matrix(1,0,0,-1,23.0421,36.7818)",
        "matrix(1,0,0,-1,63.479,23.614602)","matrix(1,0,0,-1,63.479,36.8538)","matrix(1,0,0,-1,50,36.8538)",
        "matrix(1,0,0,-1,36.521,50.1649)","matrix(1,0,0,-1,36.521,63.404)","matrix(1,0,0,-1,23.0421,63.404)",
        "matrix(1,0,0,-1,90.4369,50.1649)","matrix(1,0,0,-1,90.4369,63.404)","matrix(1,0,0,-1,76.9579,63.404)",
        "matrix(1,0,0,-1,63.479,76.4573)","matrix(1,0,0,-1,63.479,89.6965)","matrix(1,0,0,-1,50,89.6965)",
      ].map((tf, i) => (
        <path key={i} transform={tf} d="M0 0V13.239L-13.479 0Z" fill={color} />
      ))}
    </svg>
  );
}
...
// FOMA wordmark (exact brand artwork)
function FomaWordmark({ height = 24, color = T.ink }) {
  const w = (297.24 / 82.29) * height;
  return (
    <svg height={height} width={w} viewBox="0 0 297.24 82.29" xmlns="http://www.w3.org/2000/svg"
      role="img" aria-label="FOMA" style={{ display: "block", flexShrink: 0 }}>
      <path d="M0,79.09V8.49C0,4.4,3.31,1.08,7.4,1.08h41.32c.41,0,.74.33.74.74v7.84c0,.41-.33.74-.74.74H6.52c-.65,0-.99-.79-.53-1.25l2.28-2.33c.46-.47,1.27-.15,1.27.52v32.53c0,.66-.8.99-1.27.52l-2.28-2.33c-.46-.47-.13-1.25.53-1.25h40.21c.41,0,.74.33.74.74v7.62c0,.41-.33.74-.74.74H6.52c-.65,0-.99-.79-.53-1.25l2.28-2.33c.46-.47,1.27-.15,1.27.52v36.27c0,.41-.33.74-.74.74H.74c-.41,0-.74-.33-.74-.74Z" fill={color} />
      <path d="M93.49,82.29c-7.47,0-13.88-1.65-19.24-4.94-5.36-3.29-9.48-8-12.37-14.14-2.88-6.14-4.33-13.46-4.33-21.96s1.44-15.84,4.33-22.02c2.88-6.17,7-10.92,12.37-14.25,5.36-3.33,11.77-4.99,19.24-4.99s13.99,1.66,19.35,4.99c5.36,3.33,9.48,8.08,12.37,14.25,2.88,6.18,4.33,13.51,4.33,22.02s-1.44,15.82-4.33,21.96c-2.88,6.14-7.01,10.85-12.37,14.14-5.36,3.29-11.81,4.94-19.35,4.94ZM93.49,72.98c5.47,0,10.15-1.26,14.03-3.77,3.88-2.51,6.86-6.14,8.93-10.87,2.07-4.73,3.11-10.42,3.11-17.08s-1.04-12.37-3.11-17.13c-2.07-4.77-5.05-8.43-8.93-10.98s-8.56-3.83-14.03-3.83-10.04,1.27-13.92,3.83-6.86,6.21-8.93,10.98c-2.07,4.77-3.1,10.48-3.1,17.13s1.03,12.35,3.1,17.08c2.07,4.73,5.05,8.36,8.93,10.87,3.88,2.51,8.52,3.77,13.92,3.77Z" fill={color} />
      <path d="M141.4,79.77V7.32c0-3.06,2.48-5.55,5.55-5.55h7.24c.31,0,.59.19.7.49l24.29,65.93c.24.65,1.16.65,1.4,0l23.31-63.53c.63-1.73,2.28-2.88,4.12-2.88h9.5c.41,0,.75.33.75.75v77.25c0,.41-.33.75-.75.75h-8.05c-.41,0-.75-.33-.75-.75V21.18c0-.33-.46-.41-.57-.1l-21.76,58.95c-.11.29-.39.49-.7.49h-11.71c-.31,0-.59-.19-.7-.49l-21.76-58.95c-.11-.31-.58-.23-.58.1v58.59c0,.41-.33.75-.75.75h-8.05c-.41,0-.75-.33-.75-.75Z" fill={color} />
      <path d="M228.27,79.52L254.98,5.43c.79-2.2,2.88-3.66,5.21-3.66h8.45c.32,0,.6.2.7.49l27.85,77.25c.18.49-.19,1-.7,1h-8.83c-.32,0-.6-.2-.71-.5l-7.52-21.4c-.11-.3-.39-.5-.71-.5h-31.99c-.32,0-.6.2-.71.5l-7.52,21.4c-.11.3-.39.5-.71.5h-8.83c-.52,0-.88-.51-.7-1ZM249.85,48.8h25.76c.21,0,.36-.21.29-.4l-12.89-37.48c-.09-.27-.47-.27-.57,0l-12.89,37.48c-.07.2.08.4.29.4Z" fill={color} />
    </svg>
  );
}
...
function Btn({ children, kind = "ghost", small, onClick, disabled, title, ariaLabel }) {
  ...
  if (kind === "primary") { base.background = T.accent; base.color = T.ink; base.border = `1px solid ${T.accent}`; }
...
function StudioApp() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.paper, color: T.ink, fontFamily: SANS }}>
      <style>{`
        :focus-visible { outline: 2px solid ${T.accentInk} !important; outline-offset: 2px !important; border-radius: 4px; }
        ...
        <header style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between",
          ...
          <div>
            <h1 style={{ display: "flex", alignItems: "center", gap: 12, margin: 0 }}>
              <FomaWordmark height={24} color={T.ink} />
              <FomaMark size={26} color={T.accent} />
            </h1>
            <div style={{ fontFamily: MONO, fontSize: 11, color: T.muted, marginTop: 8 }}>
              studio management · home / applications / calendar / contacts
            </div>
          </div>
...
function SyncBadge({ state, lastSync, onRefresh }) {
  ...
  return (
    <button type="button" onClick={onRefresh} title="Refresh shared data"
      ...
      <span aria-hidden="true" style={{ fontFamily: SANS, fontSize: 11, fontWeight: 700, color: T.accentInk }}>↻</span>
    </button>
  );
}
      ...
      <section aria-label="This week">
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 10 }}>
          ...
          <button type="button" onClick={() => goto("calendar")}
            style={{ fontFamily: MONO, fontSize: 11, color: T.accentInk, background: "none", border: "none", cursor: "pointer" }}>
            full calendar →
          </button>
        </div>
        {weekItems.length === 0 ? (
    ...
    <div>
      ...
      {sorted.length === 0 && !adding ? (
        <Empty title="No applications yet"
          body="Add the residencies you're applying to. Everything here is shared, so your co-founder sees the same list." />
      ) : (
        <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 10, overflow: "hidden" }}>
          {sorted.map((r, i) => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            {r.url ? (
              <a href={r.url} target="_blank" rel="noreferrer" aria-label={`Open official page for ${r.name} in a new tab`}
                style={{ fontFamily: MONO, fontSize: 12, color: T.accentInk, textDecoration: "none" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
          <button type="button" onClick={() => step(-1)} aria-label="Previous month"
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: T.accentInk, padding: "2px 8px", lineHeight: 1 }}>‹</button>
            ...
            <button key={c.date} type="button" role="gridcell" onClick={() => setSelected(c.date)}
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <span style={{ fontFamily: SANS, fontSize: 12.5, fontWeight: 700,
                  ...
                  background: isToday ? T.accent : "transparent", color: isToday ? T.ink : T.ink }}>
                  {c.day}
                </span>
              </div>
                    ...
                    {e.kind === "deadline" ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3, flexWrap: "wrap" }}>
                        ...
                        <button type="button" onClick={onGoto}
                          style={{ fontFamily: MONO, fontSize: 11, color: T.accentInk, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                          open in pipeline →
                        </button>
                      </div>
                    ) : (
                      <>
                        ...
                        <div style={{ display: "flex", gap: 12, marginTop: 4 }}>
                          <button type="button" onClick={() => { setEditId(e.id); setAdding(false); }}
                            style={{ fontFamily: MONO, fontSize: 11, color: T.accentInk, background: "none", border: "none", cursor: "pointer", padding: 0 }}>edit</button>
    ...
    <div>
      ...
      {sorted.length === 0 && !adding ? (
        <Empty title="No contacts yet"
          body="Keep everyone connected to your applications in one shared place: coordinators, references, and past residents who'll answer questions." />
      ) : (
        <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 10, overflow: "hidden" }}>
          {sorted.map((c, i) => (
            <div key={c.id} style={{ borderBottom: i === sorted.length - 1 ? "none" : `1px solid ${T.line}` }}>
              <div onClick={() => setOpenId(openId === c.id ? null : c.id)}
                ...
                {c.relatedTo && (
                  <span style={{ fontFamily: SANS, fontSize: 11, fontWeight: 700, color: T.accentInk,
    ...
    <div style={{ minHeight: "100vh", background: T.paper, color: T.ink, fontFamily: SANS,
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        <div style={{ fontFamily: MONO, fontSize: 11, color: T.muted, marginBottom: 22 }}>
          studio management · sign in to continue
        </div>
        <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 12, padding: 20 }}>
          ...
          <button type="button" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setMsg(null); }}
            style={{ fontFamily: MONO, fontSize: 11, color: T.accentInk, background: "none", border: "none",
